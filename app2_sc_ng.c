/*----------------------------------------------------------------------------*/
/* app2_sc_ng.c - rutinas para Stock Cutting NO Guillotina                    */
/*----------------------------------------------------------------------------*/
...
// inicial es un puntero apuntando al nodo final de la lista
{
  	TListaPE2 *LAux2;
   	...
   	if((LAux2 = (TListaPE2*) malloc(sizeof(TListaPE2))) == NULL)
		nomemory("Laux2 en app2_agregalistap_sc_ng");
#ifdef _DEBUG_MALLOC_		
   printf("Malloc, App2_sc_ng.c, 35, LAux2, %d\n", sizeof(TListaPE2));
#endif
...
//Función que genera el layout de salida
{
	...
	fprintf(outfp,"29 %d %d %d\n", TotalPie, AnchoPl, AltoPl); //36 parmetros => 35 + 1 que indica Total de piezas + perdidas generadas... 
  	strtot[0] = 0;
    if(tipo_problema == 0) 
...
//funcin evaluacin para desplegar al mejor individuo
{
	...
	//Me estoy saltando las piezas de ancho y largo = 0
   	for(i=0;i<NumPie;i++){
      	...
      	if((Acan != 0) && (Acal != 0) && (Acan <= AnchoPl) && (Acal <= AltoPl)) {
			app2_agregalistap_sc_ng(0, 0, Acan, Acal, 'G', Acpi);
	...
	if(siguiente<NumPie){
        	...
        	Nuan = piezas[i].ancho;
         	...
         	if((Nuan != 0) && (Nual != 0) && (Nuan <= AnchoPl) && (Nual <= AltoPl)){
			    ...
			    if (Arreglo_donde_empieza == -1) {
	                ...
	                app2_agregalistap_sc_ng(Arreglo_donde_empieza, Arreglo[Arreglo_donde_empieza], Nuan, Nual, 'G', Nupi);
   	...
   	// LLENA archivo de resultados para el problema particular
 	app2_genera_layout_sc_ng(tipo_problema, nomarch, time_consumtion, Bus2);
 	
}//End app2_funceval_sc_ng
...
// Funcin Objetivo, transforma el string cromosoma en un arreglo de piezas
{
	...
	TNodoAP  	*piezaschromo; // variable que apunta a todas las piezas del cromosoma generado
   	...
   	if((piezaschromo = (TNodoAP*) malloc(NumPie*sizeof(TNodoAP))) == NULL)
		nomemory("piezaschromo en app2_objfunc_sc_ng");
#ifdef _DEBUG_MALLOC_		
   printf("Malloc, App2_sc_ng.c, 365, piezaschromo, %d\n", NumPie*sizeof(TNodoAP));
#endif
  	...
  	//Asigna cada pieza del arreglo piezasproblema en arregla piezaschromo dependiendo del string chrom
   	...
   	app2_funceval_sc_ng(tipo_problema, nomarch, time_consumtion, piezaschromo);
   	free(piezaschromo);
#ifdef _DEBUG_MALLOC_		
   printf("Free, app2_ng.c, 417, piezaschromo\n");
#endif
}//End app2_objfunc_sc_ng
