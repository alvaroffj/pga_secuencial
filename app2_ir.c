...
void app2_genera_layout_ir(int tipo_problema, char *nombrearchivo, cpu_consummed_time_t *t)
//Función que genera el layout de salida
{
	char strtot[32768], straux[100];
	
	//Primero coloca parámetros importantes 
	fprintf(outfp,"22 %d %lf %lf\n", uCantidadPiezas, bestfit.PEval.var1, dAltoLamina); //22 líneas => 21 parámetros + 1 (fin de parámetros) 
  	strtot[0] = 0;
    if(tipo_problema == 0) 
	...
	strcat(strtot, straux);
	sprintf(straux,"ANCHO_LAMINA (cm)              : %lf\n", bestfit.PEval.var1);
	strcat(strtot, straux);
	sprintf(straux,"LARGO_LAMINA (cm)              : %lf\n", dAltoLamina);
	strcat(strtot, straux);
...
// Función de evaluación: Determina el área que ocupa el perfil...
{
	...
	//Eval.perdida = (6.35 * dMaximoX) - 144; //Escalamiento
    ...
    Eval.calidad = (( dAreaPerfil - dAreaPiezasEncajadas ) / (dMaximoX * dAltoLamina)) ;

	return Eval; 
}//End app2_funceval_ir

void app2_objfunc_ir(int tipo_problema, char *nomarch, cpu_consummed_time_t *time_consumtion, struct bestever *bestcritter)
// Función Objetivo, transforma el string cromosoma en un arreglo de piezas
{
	...
	//app2_genera_layout_ir(tipo_problema, nomarch, time_consumtion);
