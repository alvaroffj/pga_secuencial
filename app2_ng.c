/*----------------------------------------------------------------------------*/
/* app2_ng.c - rutinas para Problema de Corte de Piezas NO Guillotina         */
/*----------------------------------------------------------------------------*/
...
//funcin evaluacin para desplegar al mejor individuo
{
	...
	if(siguiente<NumPie){
 	...
 	app2_genera_layout_ng(tipo_problema, nomarch, time_consumtion, Bus2);
 	
}//End app2_funceval_ng
...
// Funcin Objetivo, transforma el string cromosoma en un arreglo de piezas
{
   ...
   printf("Malloc, App2_ng.c, 365, piezaschromo, %d\n", NumPie*sizeof(TNodoAP));
  	...
  	//Asigna cada pieza del arreglo piezasproblema en arregla piezaschromo dependiendo del string chrom
   	...
   	app2_funceval_ng(tipo_problema, nomarch, time_consumtion, piezaschromo);
   	free(piezaschromo);
#ifdef _DEBUG_MALLOC_		
